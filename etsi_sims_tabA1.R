...
sim_function_A <- function(setting) {
  ...
  results <- matrix(ncol = 5, nrow = 5)
  if (setting == 1 | setting == 2 | setting == 4 |setting == 5) {
    rownames(results) <- c("Point Estimate", "ESE", "ASE", "Effect Size", "Power")  
  } else if (setting == 3) {
    rownames(results) <- c("Point Estimate", "ESE", "ASE", "Effect Size", "Type 1 Error")
  }
...
tabA1_setting4 <- sim_function_A(4)
tabA1_setting5 <- sim_function_A(5)

tabA1 <- rbind(tabA1_setting4, tabA1_setting5)
saveRDS(tabA1, file = "results/tabA1.rds")
